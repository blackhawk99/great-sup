...
const BeachDetailView = ({ 
  ...
  // Handle update forecast button - FIXED with onDataUpdate check
  const handleUpdateForecast = async () => {
    ...
    try {
      ...
      if (typeof onDataUpdate === 'function') {  // Check if it's a function before calling
        onDataUpdate();
      }
    } catch (err) {
    ...
    return (
      <div className="bg-white rounded-lg p-5 border shadow-sm mt-4">
        ...
        <div className="space-y-3">
          {hours.map(hour => {
            // Make sure we have data for this hour
            if (!weatherData.hourly.windspeed_10m[hour]) return null;
            ...
            {debugMode && (
              <div className="mb-4 p-2 bg-gray-100 rounded text-xs">
                <p>Debug info: Score: {score !== null ? score : 'null'}, 
                  ...
                  Wind values: {weatherData.hourly.windspeed_10m ? 'yes' : 'no'},
              ...
              <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-md mb-4">
                ...
                <div className="grid grid-cols-2 gap-3 mt-4">
                  <div className="bg-white rounded-lg p-3 border">
                    ...
                    <p>{weatherData.hourly && weatherData.hourly.windspeed_10m && weatherData.hourly.windspeed_10m.length > 12 ? Math.round(weatherData.hourly.windspeed_10m[12]) : 'N/A'} km/h</p>
                  </div>
                  <div className="bg-white rounded-lg p-3 border">
                    <h3 className="font-medium mb-2">Temperature</h3>
                    <p>{weatherData.hourly && weatherData.hourly.temperature_2m && weatherData.hourly.temperature_2m.length > 12 ? Math.round(weatherData.hourly.temperature_2m[12]) : 'N/A'}°C</p>
                  </div>
                </div>
